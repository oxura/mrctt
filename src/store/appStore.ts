...
const getCurrentMonthSales = (payments: PaymentRecord[]): number => {
  const now = new Date();
  ...
  persist(
    immer((set, get) => ({
      ...
      kpi: {
        ...
        inProgress: 14,
        sales: getCurrentMonthSales(mockPayments),
        overdueTasks: 2,
      },
      onboarding: {
        companyName: '',
      ...
      completeOnboarding: (company, user) =>
        set((state) => {
          const newCompany = {
            ...company,
            modules: state.onboarding.modules.map((mod) => ({ ...mod })),
          };
      ...
      refreshKpi: () =>
        set((state) => {
          ...
          const sales = getCurrentMonthSales(state.billing.payments);
          state.kpi.inProgress = inProgress;
          state.kpi.overdueTasks = overdueTasks;
          state.kpi.sales = sales;
        }),
    })),
    {
      name: 'app-storage',
